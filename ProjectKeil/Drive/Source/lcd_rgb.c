...
//	参数：x、y为起点坐标，width - 长度，height - 宽度
//
void LCD_FillRect_A(u16 x, u16 y, u16 width, u16 height)
{
	DMA2D_InitTypeDef   DMA2D_InitStruct;
	u32 Display_Add = 0;					//当前层的显存地址
	u8 Display_ColorMode = 0;			//当前层的颜色格式
...
* Return         : None
*******************************************************************************/
void LCD_WriteBMP( u16 sx,u16 sy,u16 ex,u16 ey,const  u8   *bitmap)
{
	vu32 index,j;
  vu32 size = LCD_Pixels;
  u8 *bitmap_ptr = (u8 *)bitmap;
  u8 point1,point2,point3;
	u32 color_get=0;
	...
	u16 add_ey=ey;
		...
		for(j=add_sx;j<=add_ex;j++)
//		for(j=add_sy;j<=add_ey;j++)
		{
				for(index = add_sy; index <=add_ey; index++)
//				for(index = add_sx; index <=add_ex; index++)
				{
					point1= *bitmap_ptr++;   
					point2= *bitmap_ptr++; 
					point3= *bitmap_ptr++; 
					
					color_get = (point1<<16) + (point2<<8) +(point3);
					LCD_DrawPoint(index,j,color_get);	
				}
		}
	/*
		add_sx=sx;
...
void LCD_FLICKER_A(void)
{
  ...
  u8 point1,point2,point3;
	u32 color_get=0;
		...
		for(j=0;j<=LCD_Height-1;j+=2)
		{
				for(index = 0; index <=LCD_Width-1; index+=2)
				{
					point1= 0x80;   
					point2= 0x00; 
					point3= 0x80;
					...
					color_get = (point1<<16) + (point2<<8) +(point3);
					LCD_DrawPoint(index+1,j,color_get);	
				}
				for(index = 0; index <=LCD_Width-1; index+=2)
				{
					point1= 0x00;   
					...
					color_get = (point1<<16) + (point2<<8) +(point3);
					LCD_DrawPoint(index,j+1,color_get);	
					...
					color_get = (point1<<16) + (point2<<8) +(point3);
					LCD_DrawPoint(index+1,j+1,color_get);	
				}

		}
}

void LCD_FLICKER_B(void)
{
	vu32 index,j;
  u8 point1,point2,point3;
	u32 color_get=0;
